...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // CASO ESPECIAL HOME:
    // Si estamos en home (pathname = /) y encontramos "Somos ITCA" activo,
    // le quitamos la clase active para forzar el logo si estamos arriba.
    // Esto es para cumplir el requerimiento de que en Home default sea Logo.
    if (window.location.pathname === '/' && initialActive && initialActive.textContent.trim() === 'Somos ITCA' && window.scrollY < 100) {
    ...
    // Fallback final: Si aún no hay activo, usar Logo
    if (!initialActive) {
    ...
    if (navContainer) {
        navContainer.addEventListener('mouseleave', () => {
            ...
            if (currentActive) {
                ...
                // Si no hay activo (ej: estamos en el logo o ningún link tiene clase active)

                // CASO SOMOS ITCA PAGE:
                // Si estamos en la página /somos-itca, el fallback debe ser el link "Somos ITCA", no el logo.
                if (window.location.pathname.includes('somos-itca')) {
                    const somosItcaLink = Array.from(navItems).find(link => link.href.includes('somos-itca'));
                    if (somosItcaLink) {
                        moveIndicator(somosItcaLink);
                        return;
                    }
                }

                // CASO DEFAULT / HOME:
                // Si estamos arriba o el logo fue lo último (asumido por falta de active), fallback a logo
