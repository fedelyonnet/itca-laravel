<x-app-layout>
                ...
                <div class="p-6 text-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-400 mt-1">
                                Paneles visibles: {{ $sedes->count() }}/8
                            </p>
                        </div>
                                    ...
                                    @foreach($sedes as $sede)
                                        <tr data-id="{{ $sede->id }}" class="sede-draggable hover:bg-gray-700/50 transition-colors">
                                            ...
                                            <td class="px-4 py-4 text-sm text-white">
                                                <div class="font-medium">{{ $sede->nombre }}</div>
                                            </td>
                                            
                                            <!-- Dirección -->
                                            <td class="px-4 py-4 text-sm text-gray-300">
                                                <div class="max-w-xs truncate" title="{{ $sede->direccion }}">
                                                    {{ $sede->direccion }}
                                                </div>
                                            </td>
                                            ...
                                            <!-- Estado -->
                                            <td class="px-4 py-4">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $sede->disponible ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    {{ $sede->disponible ? 'Disponible' : 'No disponible' }}
                                                </span>
                                            </td>
                                            ...
                                            <td class="px-4 py-4">
                                                <div class="flex space-x-2">
                                                    <!-- Editar -->
                                                    <div class="relative group">
                                                        <button onclick="editSede({{ $sede->id }})" 
                                                    ...
                                                    <!-- Eliminar -->
                                                    <div class="relative group">
                                                        <form action="{{ route('admin.sedes.destroy', $sede->id) }}" method="POST" onsubmit="return confirmDelete(event)" class="inline">
                                                            @csrf
                                                            @method('DELETE')
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-200">No hay sedes</h3>
                            <p class="mt-1 text-sm text-gray-300">Comienza agregando una nueva sede.</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div id="sedeModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
        <div class="flex items-center justify-center min-h-screen p-2 sm:p-4">
            <div class="bg-gray-800 rounded-lg p-1 sm:p-6 w-full max-w-[240px] sm:max-w-md">
                ...
                <form id="sedeForm" action="{{ route('admin.sedes.store') }}" method="POST" enctype="multipart/form-data" onsubmit="return handleFormSubmit(event)">
                    @csrf
    ...
    <script>
        ...
        function openModal() {
            ...
            document.getElementById('sedeForm').action = '{{ route("admin.sedes.store") }}';
            isEditMode = false;
        }
