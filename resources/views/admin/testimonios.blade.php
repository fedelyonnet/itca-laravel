<x-app-layout>
                ...
                <div class="p-6 text-gray-100">
                    ...
                    @if($testimonios->count() > 0)
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-700">
                                ...
                                <tbody id="sortable-testimonios" class="bg-gray-800 divide-y divide-gray-700">
                                    @foreach($testimonios as $testimonio)
                                        <tr data-id="{{ $testimonio->id }}" class="testimonio-draggable hover:bg-gray-700/50 transition-colors">
                                            <!-- Drag Handle -->
                                            <td class="px-2 py-4">
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-200">No hay testimonios</h3>
                            <p class="mt-1 text-sm text-gray-300">Comienza agregando un nuevo testimonio.</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="modalTexto" name="texto" rows="3" 
                              ...
                              placeholder="Escribí el testimonio completo..."></textarea>
                </div>
    ...
    <script>
        ...
        // Sortable for Testimonios
        document.addEventListener('DOMContentLoaded', function() {
            const testimoniosList = document.getElementById('sortable-testimonios');
            if (testimoniosList && typeof Sortable !== 'undefined') {
                new Sortable(testimoniosList, {
                    animation: 150,
                    handle: '.drag-handle',
                    draggable: '.testimonio-draggable',
                    ghostClass: 'opacity-50',
                    chosenClass: 'bg-blue-900/30',
                    dragClass: 'opacity-75',
                    onEnd: function (evt) {
                        const items = Array.from(testimoniosList.querySelectorAll('.testimonio-draggable'));
                        const orderedIds = items.map(item => item.getAttribute('data-id'));
                        
                        fetch('{{ route("admin.testimonios.update-order") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
