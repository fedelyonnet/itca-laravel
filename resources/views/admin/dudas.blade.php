<x-app-layout>
                ...
                <div class="p-6 text-gray-100">
                    ...
                    @if($dudas->count() > 0)
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-700">
                                ...
                                <tbody id="sortable-dudas" class="bg-gray-800 divide-y divide-gray-700">
                                    @foreach($dudas->sortBy('orden') as $duda)
                                        <tr data-id="{{ $duda->id }}" class="duda-draggable hover:bg-gray-700/50 transition-colors">
                                            <!-- Drag Handle -->
                                            <td class="px-2 py-4">
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-200">No hay FAQs</h3>
                            <p class="mt-1 text-sm text-gray-300">Comienza agregando tu primera FAQ.</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div id="modalAgregarDuda" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const dudasList = document.getElementById('sortable-dudas');
            if (dudasList && typeof Sortable !== 'undefined') {
                new Sortable(dudasList, {
                    animation: 150,
                    handle: '.drag-handle',
                    draggable: '.duda-draggable',
                    ghostClass: 'opacity-50',
                    chosenClass: 'bg-blue-900/30',
                    dragClass: 'opacity-75',
                    onEnd: function (evt) {
                        const items = Array.from(dudasList.querySelectorAll('.duda-draggable'));
                        const orderedIds = items.map(item => item.getAttribute('data-id'));
                        
                        fetch('{{ route("admin.dudas.update-order") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
