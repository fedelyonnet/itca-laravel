...
class StoreNoticiaRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'fecha_publicacion' => 'required|string',
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->fecha_publicacion) {
            ...
            if ($fecha) {
                ...
                // Si no se puede convertir, intentar con el formato original
                try {
                    $fecha = new \DateTime($this->fecha_publicacion);
                    $this->merge([
                        'fecha_publicacion' => $fecha->format('Y-m-d')
                    ]);
                } catch (\Exception $e) {
                    // Si no se puede convertir, dejar como está para que falle la validación
                }
            }
        }
    }
